...
def holi(to_mins,route):
	if(to_mins>=480 and to_mins<540):
        	...
        	traffic=pd.read_csv('sloth4.csv')
        	X=traffic['to_minutes'].values
        	X=X.reshape(-1,1)
	elif(to_mins>=720 and to_mins<780):
		traffic=pd.read_csv('sloth5.csv')
        	X=traffic['to_minutes'].values
        	X=X.reshape(-1,1)
	elif(to_mins>=780 and to_mins<840):
		traffic=pd.read_csv('sloth6.csv')
        	X=traffic['to_minutes'].values
        	X=X.reshape(-1,1)
  	elif(to_mins>=840 and to_mins<900):      	
        	traffic=pd.read_csv('sloth7.csv')
        	X=traffic['to_minutes'].values
        	X=X.reshape(-1,1)	
        elif(to_mins>=900 and to_mins<960):
        	traffic=pd.read_csv('sloth8.csv')
        	X=traffic['to_minutes'].values
        	X=X.reshape(-1,1)
        elif(to_mins>=960 and to_mins<1020):
        	traffic=pd.read_csv('sloth9.csv')
        	X=traffic['to_minutes'].values
        	X=X.reshape(-1,1)
    	elif(to_mins>=1020 and to_mins<1080):
        	traffic=pd.read_csv('sloth10.csv')
        	X=traffic['to_minutes'].values
        	X=X.reshape(-1,1)
    	elif(to_mins>=1080 and to_mins<1140):
        ...
        if True:
            #try:
                ...
                if data:
                    ...
                    stry=response.split(':')
                    #response.decode('unicode_escape').encode('ascii','ignore')
                    #print(stry)
        	    ...
        	    p1=pessimistic(t,r)
                    ...
                    z,k,j,g,p=graph(t,r,holid)
                    #print(z,k,j,g,p)
                    #graph(t,r)
                    data_to_send0="{\"fromserver\":{\"Car_best_guess\":"+str(a)+",\"Car_opt\":"+str(o1)+",\"Car_pess\":"+str(p1)+",\"Bus\":"+str(b)+",\"X_axis\":"+str(z[:])+",\"best_guess\":"+str(k[:])+",\"optimistic\":"+str(j[:])+",\"pessimistic\":"+str(g[:])+",\"busg\":"+str(p[:])+"}}"
                    
                    #data_to_send1="{'Car_best_guess':"+str(a)+",'Car_opt':"+str(o1)+",'Car_pess':"+str(p1)+",'Bus':"+str(b)
                    '''
                    car={'Car_best_guess':str(a)}
                    car_send=str(car)
                    
                    data_to_send2="'X_axis':"+str(z[:])
                    data_to_send3=",'best_guess':"+str(k[:])                   
                    data_to_send4=",'optimistic':"+str(j[:])
                    data_to_send5=",'pessimistic':"+str(g[:])
                    data_to_send6=",'busg':"+str(p[:])+"}"
                    '''
                    #data_to_send1=str(data_to_send0)
                    data_to_send=data_to_send0
                    #data_to_send11="\x00"+data_to_send2
                    print (data_to_send)
                    client.send(data_to_send)
                    #client.send(data_to_send11)
                    client.close()
                else:
...
if __name__ == "__main__":
    ...
    ThreadedServer('192.168.43.241',3345).listen()
